...
export default function DriverProfilePage() {
  ...
  useEffect(() => {
    (async () => {
      try { const r = await fetch("/api/driver/subscription"); if (r.ok) setSub(await r.json()); } catch {}
    })();
  }, []);
  ...
  return (
    <div className="min-h-screen bg-[#F9F9F9] pb-24">
      <header className="bg-white sticky top-0 z-50 border-b border-gray-200 flex flex-row-reverse items-center px-4 h-16">
        <Link href="/driver/dashboard" className="p-2 hover:bg-gray-100 rounded-full">
      ...
      <main className="max-w-lg mx-auto px-4 py-8 space-y-4">
        <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm text-center">
          ...
          <p className="text-gray-400 text-xs mt-1">سائق</p>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
          ...
          <div className="flex items-center justify-between">
            ...
            <span className={`px-3 py-1 rounded-full text-xs font-bold ${
              ...
              sub?.subscriptionStatus === "pending" ? "bg-yellow-100 text-yellow-700" :
              "bg-red-100 text-red-700"
            }`}>
              {sub?.subscriptionStatus === "active" ? "نشط" :
               sub?.subscriptionStatus === "pending" ? "معلق" : "غير نشط"}
            </span>
          </div>
          {days !== null && (
            <div className="flex items-center justify-between mt-3">
              <span className="text-sm text-gray-500">المتبقي</span>
              <span className={`text-sm font-bold ${days <= 5 ? "text-red-500" : "text-green-600"}`}>
        ...
        <button onClick={() => signOut({ redirect: true, callbackUrl: "/login" })}
          className="w-full py-4 bg-red-600 hover:bg-red-700 text-white font-bold rounded-xl transition-all shadow-md text-center cursor-pointer text-base">
          تسجيل الخروج
        </button>
      </main>
    </div>
